...
let userInventory = loadUserInventory();
...
console.log(`UserInven : ${userInventory.join(', ')}`);


// 초기 userInventory 값을 로드하는 함수
function loadUserInventory() {
  // 로컬 스토리지에서 userInventory 불러오기
  let storedUserInventory = window.localStorage.getItem("UserInven");

  // JSON 문자열을 배열로 변환하거나, 저장된 값이 없으면 빈 배열을 반환
  return storedUserInventory ? JSON.parse(storedUserInventory) : [];
}

// 새로운 chest_selected 배열 값을 추가하는 함수
function addChestToInventory(chest_selected) {
  // 기존 userInventory 로드
  // let userInventory = loadUserInventory();
  // chest_selected 값을 userInventory에 추가
  userInventory.push(chest_selected);
  // 업데이트된 userInventory를 다시 로컬 스토리지에 저장
  window.localStorage.setItem("UserInven", JSON.stringify(userInventory));
  // 콘솔에 업데이트된 userInventory 출력
  // console.log(`UserInven : ${userInventory.join(', ')}`);
}
...
const chest = () => {
  ...
  // 상자보상
  const dice0 = Math.floor(Math.random() * 100) + 1;
  if (dice0) {
    console.log(dice0);
    if (dice0 >= 1 && dice0 <= 55) {
      nm();
      console.log(chest_selected);
      array.push("nm");
    } else if (dice0 >= 56 && dice0 <= 75) {
      mg();
      console.log(chest_selected);
      array.push("mg");
    } else if (dice0 >= 76 && dice0 <= 90) {
      ra();
      console.log(chest_selected);
      array.push("ra");
    } else if (dice0 >= 91 && dice0 <= 99) {
      ep();
      console.log(chest_selected);
      array.push("ep");
    } else if (dice0 === 100) {
  ...
  // 배열 생성
  // let userInventory = array;
  // window.localStorage.setItem("UserInv", JSON.stringify(userInventory));
  // // 로컬 스토리지에서 JSON 문자열을 불러오기
  // let storedUserInventory = window.localStorage.getItem("UserInv");
  // // JSON 문자열을 배열로 변환
  // if (storedUserInventory) {
  //     userInventory = JSON.parse(storedUserInventory);
  //     console.log(userInventory); // ["item1", "item2", "item3"]
  // }
...
function trun(e) {
  ...
  if (user.hp <= 0) {
    ...
    addChestToInventory(chest_selected);
